...
module.exports = class Comment {
    ...
    static async insert({ body, parentCommentId, userId, postId, parentUserId }) {
        const { rows } = await pool.query(`
        WITH inserted_comment as (
            INSERT INTO comments (body, vote_score, date_created, read_by_parent, parent_comment_id, user_id, post_id, parent_user_id)
        ...
        `, [body, 0, Date.now(), false, parentCommentId, userId, postId, parentUserId])
    ...
    static async findRepliesByUserId(userId) {
        const { rows } = await pool.query(`
        select comments.*, display_name as "createdBy"
        from comments
        join users
        ...
        return rows.map(row => Comment.commentsMunge(row))
    }

    static async findUnreadCommentReplies(userId) {

        const { rows } = await pool.query(`
        select comments.*, json_agg(c2) as comments
        from comments
        join comments as c2
        ...
        return rows.map(row => (
            {
                ...new Comment(row),
                unreadReplies: row.comments.map(row => new Comment(row))
            }))
    }
