...
module.exports = class Post {
    ...
    static async find(page = 1, orderType = 'top') {
        ...
        SELECT * from post_count, all_posts
        `, [limit, offset])

        console.log(rows)
        if (!rows[0]) throw new Error('no posts found..')
        return {
            page,
            count: rows[0].count,
            postArray: rows.map(row => ({
                ...new Post(row),
    ...
    static async findById(id) {
        ...
        from posts
        left join comments
        on posts.id = comments.post_id AND comments.parent_comment_id IS NULL
        inner join users
        on posts.user_id = users.id
        inner join boards
        on posts.board_id = boards.id
    ...
    static async findUnreadPostReplies(userId) {
